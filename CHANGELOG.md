...
- **`references/trainer-github-pr-commentary.md`** — mandatory PR body test-plan granularity and PR comment **`### Trainer notes`** (Program notes · Your form · Next session); forbids Pedagogy heading (aligned with `_sdk_codereview.txt`).
...
- **CI gate + commentary** — `ci-trainer-pr-review-gate.sh` requires `### Trainer notes` and **rejects** `### Pedagogy` (fixes drift from SDK codereview prompt).
...
- **`references/sdk-merge-codereview-gate.md`**, **`cursor-sdk-playground/prompts/_sdk_codereview.txt`** — require granular test plans + Trainer notes before APPROVE.
