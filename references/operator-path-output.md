...
```markdown
See `~/Projects/toren/applications/dropzone-ai/iii.reference/strategy_dropzone-ai.md` for the strategy.
```
...
4. **Use `~/` prefix** — e.g. `~/Projects/toren/...` not `/Users/dubs/Projects/...`
...
| Format | Result | Why |
|--------|--------|-----|
| `` `~/Projects/toren/.../strategy.md` `` | ✅ Clickable, opens in editor | File path in inline code — detected by PR #31407 |
| `[Strategy](~/Projects/toren/.../strategy.md)` | ❌ Plain hyperlink | Markdown link doesn't get file-link treatment |
| `~/Projects/toren/.../strategy.md` | ❌ Plain text | No backticks = not detected |
| `` `strategy.md` `` | ⚠️ May work if relative path resolves | Short path, context-dependent |
| `` `~/Projects/toren/.../folder` `` | ❌ No `.` in path | Missing file extension, not detected as file |
...
| File | Description |
|------|-------------|
| `~/Projects/toren/.../strategy.md` | Strategy doc |
| `~/Projects/toren/.../dossier.md` | Engram dossier |
```

### In lists

```markdown
- `~/Projects/toren/applications/dropzone-ai/iii.reference/strategy_dropzone-ai.md` — Strategy
- `~/Projects/toren/applications/dropzone-ai/ii.recruiter_interview/recruiter_screen_dropzone-ai.md` — Cheat sheet
```

### Prose references

```markdown
The recruiter cheat sheet is at `~/Projects/toren/applications/dropzone-ai/ii.recruiter_interview/recruiter_screen_dropzone-ai.md`.
```
...
bash ~/Projects/toren/applications/dropzone-ai/open_dropzone_docs.sh

# Reveal all docs in Finder
bash ~/Projects/toren/applications/dropzone-ai/reveal_dropzone_docs.sh
```
