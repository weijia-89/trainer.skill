...
1. **trainer** — coach stance; do not expand scope beyond the diff.
2. **form-check** — `code-review` / `adversarial-review` on the PR diff. Stakes from repo `AGENTS.md` / `CLAUDE.md`.
3. **review-rigor** — opening audit + rubric (SEC, COR, ARC, PRF, TST). Per-finding scorecard; ship threshold per stakes tier.
...
Use ranked findings `### T{n} · P{0-4} · {rubric} · conf N%` (toebeans P0–P3 must clear before APPROVE).
...
| **APPROVE** | Meets tier floor; residual nits non-blocking |
...
2. **PR comment** — one canonical comment per PR:
   ...
   - `### Findings` table + `### Trainer notes` (**Program notes**, **Your form**, **Next session**)
