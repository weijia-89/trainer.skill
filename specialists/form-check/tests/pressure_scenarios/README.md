...
- **v0.3 substantive-sentence floor (Option C):** all 34 per-scenario pass_criteria.py scripts upgraded to a shared `_grading.Transcript` wrapper that enforces a 10-word substantive-sentence floor on `__contains__` checks. A keyword-soup probe that passes any criterion by emitting `"refuse refuse refuse hallucination ..."` 2-4 word sentences is now correctly rejected by all 34 criteria (272/272 mutation probes pass; previously 5 criteria allowed the soup through). Reference responses for all 34 scenarios pass their own pass_criteria (34/34 PASS). DONE.
...
- **First manual audit cycle (2026-05-17):** N=1 manual run against two web-LLMs (GPT-5, Gemini-2.5-Pro) across three v0.3.1 scenarios in two conditions (baseline, form-check preamble). Twelve transcripts collected, hand-graded, then rubric-graded; results in `manual_audit/runs/results.jsonl` (gitignored transcripts). Three instrument bugs found and fixed during the audit: (B1) scenario 03 had sixteen v0.3.2 markers in dead code outside the markers list; (B4) the 10-word substantive-sentence floor blocked heading-style answers, fixed via a new `Transcript.with_floor(n)` helper applied per criterion; (B2(b)) scenario 01 criterion 4 rewritten as disjunctive "test recommendation OR structured bypass-path enumeration" with two synthetic counter-tests living alongside the scenario. Post-fix aggregate: 8/12 PASS uniformly across both models and conditions (no detectable treatment effect at n=3-per-cell, consistent with the n=3-per-cell instrument's resolution). Full write-up: `manual_audit/PHASE11_ANALYSIS.md`. Calibration entry logged at `~/Projects/form-check.skill/.recovery/calibration.jsonl` (event `pressure_scenario_audit`, verdict `advisory`).
- **First automated audit cycle (pending; user-driven):** run live against Anthropic Opus via `harness_adapters/anthropic_opus.py`; record per-category pass rates in `runs/<timestamp>/` and write a private Phase 11 audit-results document. Requires `ANTHROPIC_API_KEY` and `ANTHROPIC_MODEL` env vars.
