# Iron law: status check updates docs (superset orch)

**Canonical:** `superset.skill` § Status check + changelog/README iron law (v0.8.2+).

When the operator says **check status** (or EOD changelog pass):

1. Evidence: git + `gh pr` per active repo.
2. **Queue SSOT** (`sdk-ssot-1.md` history + `sdk-ssot-2.md` kickoffs for SDK weekend): full update + push playground.
...
Workers propose changelog bullets in `localonly/daily/`; orch merges on status check.
