...
| H16 | Second+ agent on the **same git repo**: pre-spawn integration check run (`git fetch`, branch inventory, `git merge-tree` or serial merge-back in manifest)? Post-agent: PR `mergeable` or worktree `merge --no-commit --no-ff origin/main` dry-run before opening a second PR? (added v0.8.4; anchored to toebeans 2026-05-23 B+E parallel dispatch, PR #42 `CONFLICTING` on `.codeit/calibration.jsonl` despite disjoint module `owned_paths`) | For each repo with 2+ agents in the batch: confirm coordination notes or daily log record merge-tree output; before slot 2, first agent's PR `gh pr view --json mergeable` is `MERGEABLE` OR manifest has `depends_on` + later phase for serial merge-back | Run gates in `SKILL.md` § Same-repo parallel agents: main integration gate; add shared-parent files to implicit scope; defer agent 2 or rebase agent 1 first |
| H17 | **2+ open PRs** on one repo without merge-train plan: all branches still based on stale `main`, or hot-file overlap across PRs? After any merge to `main`, were remaining PRs rebased and trainer `head=` updated? (added v0.8.6; anchored to toebeans 2026-05-28 stacked PRs #74–#79) | List open PRs + base SHA vs `origin/main`; confirm daily log / SSOT names merge order; after each merge, log rebase + `mergeable` check per remaining PR | Run `SKILL.md` § Iron law: stacked PRs and merge train; use one PR per slice or `integrate/*`; never N diverged feature PRs on same hot files |
...
- **Same-repo integration map (v0.8.4).** For each git repo with 2+ agents, list implicit shared parents (`CHANGELOG.md`, calibration JSONL, lockfiles, version catalogs) plus `git merge-tree` between open worker branches. Disjoint module `owned_paths` is not enough; see H16 and `SKILL.md` § Same-repo parallel agents: main integration gate.
- **Open PR merge train (v0.8.6).** For each repo with 2+ open PRs, record merge order in SSOT; after each lands on `main`, rebase remaining branches before opening another PR on the same hot files. See H17 and `SKILL.md` § Iron law: stacked PRs and merge train.
