...
```bash
# Lexical-signal pass
python3 ~/Projects/deai.skill/deai-scan.py <PROSE_FILE> 2>&1 | tee /tmp/<task-slug>-deai-scan.txt

# Per-sentence band + family map
python3 ~/Projects/deai.skill/deai-check.py <PROSE_FILE> 2>&1 | tee /tmp/<task-slug>-deai-check.txt

# Baseline comparison (the canon Wei file for this genre)
python3 ~/Projects/deai.skill/deai-check.py <BASELINE_FILE> 2>&1 | tail -10
